...
function brushTimeArcLink(link) {
    ...
    function combineProp(d) {
        return `${d.source.id},${d.target.id},${d[COL_ACTION]}`;
    }
}
...
function drawTimeArc(theGroup, width, height, nodes, links, linkTypes, linkTypesColor, linkStrokeWidthScale, onNodeMouseOverCallBack, onTimeArcLinkMouseOverCallBack, orderFunction) {
    ...
    function endedCalculatingY() {
        ...
        let xScale = d3.scaleTime().domain(d3.extent(links.map(l => l[COL_TIME]))).range([0, width]);
        let nestedByNode = {};
        ...
        nodes.forEach(n => {
            let eventTimes = nestedByNode[n.id].map(l => l[COL_TIME]);
        ...
        let enterLink = linkElements.enter().append('path').attr('class', "tALinkElements")
            .attr("marker-end", d => {
                if (d.source === d.target) {
                    return `url(#markerSelfLoopTA${linkTypes.indexOf(d[COL_ACTION])})`;
                }
                return `url(#markerTA${linkTypes.indexOf(d[COL_ACTION])})`;
            })
            .attr("stroke", d => linkTypesColor(d[COL_ACTION]))
        ...
        linkElements.on("mouseover", function (theLink) {
            ...
            d3.selectAll('.tANodeTexts').transition('moveTextToMouse').duration(timeArcSettings.transition.duration).attr("x", function (d) {
                if (d === theLink.source || d === theLink.target) {
                    return `${xScale(theLink[COL_TIME])}`;
                } else {
                    return d.x;
                }
            });
        ...
        function countSiblingLinks(theLink) {
            ...
            for (let i = 0; i < links.length; ++i) {
                if ((links[i].source.id == source.id && links[i].target.id == target.id) && links[i][COL_TIME] === theLink[COL_TIME])
                    count++;
            }
            return count;
        }
